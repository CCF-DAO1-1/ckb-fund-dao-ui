...
export const buildAndSendVoteTransaction = async (
  ...
  try {
    ...
    // 调试日志：检查 votingConfig
    logger.log("votingConfig:", votingConfig);
    logger.log("votingConfig.voteTypeCodeHash:", { hash: votingConfig?.voteTypeCodeHash });
    logger.log("votingConfig.voteContractTxHash:", { hash: votingConfig?.voteContractTxHash });

    if (!votingConfig || !votingConfig.voteTypeCodeHash) {
      throw new Error("votingConfig or votingConfig.voteTypeCodeHash is undefined");
    }
    ...
    logger.log("=== TX CREATED ===");
    logger.log(ccc.stringify(tx));
    logger.log("==================");
    ...
    // 8. 植入投票证明 (Vote Proof) 到第一个 witness 中
    // 参考官方 user-vote-example.ts，解开由 CCC 生成的 witness 增加 outputType 再重新封存回去
    let witness: InstanceType<typeof WitnessArgs>;
    if (tx.witnesses[0] && tx.witnesses[0] !== '0x') {
      let witnessBytes: Uint8Array;
      if (typeof tx.witnesses[0] === 'string') {
        const { bytesFrom } = await import('@ckb-ccc/core');
        witnessBytes = bytesFrom(tx.witnesses[0]);
      } else {
        witnessBytes = new Uint8Array(tx.witnesses[0] as ArrayLike<number>);
      }
