...
// 提交验收报告参数类型
export interface SubmitAcceptanceReportParams {
  did: string; // 用户DID
  params: {
    proposal_uri: string; // 提案URI
    report: string; // 验收报告内容
    timestamp: number; // 时间戳
  };
  signed_bytes: string; // 签名字节（顶层）
  signing_key_did: string; // 签名密钥DID（顶层）
...
// 提交验收报告响应类型
export interface SubmitAcceptanceReportResponse {
  success: boolean;
  outputsData?: string[];
  vote_meta?: {
    id: number;
 ...
 */
export const submitAcceptanceReport = defineAPI<
  SubmitAcceptanceReportParams,
  SubmitAcceptanceReportResponse
>(
