...
export async function GET(req: NextRequest) {
  try {
    ...
    // 调用后端 API
    const apiUrl = `${SERVER}/api/proposal/replied?${queryParams.toString()}`;
    ...
    console.error("Error fetching replied list:", error);
    
    if (axios.isAxiosError(error)) {
      const status = error.response?.status || 500;
      const errorMessage = error.response?.data?.message || error.message || "Failed to fetch replied list";
