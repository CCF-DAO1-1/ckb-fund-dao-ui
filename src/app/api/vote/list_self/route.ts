...
export async function GET(req: NextRequest) {
  try {
    ...
    // 调用后端 API
    const apiUrl = `${SERVER}/api/vote/list_self?${queryParams.toString()}`;
    ...
    if (axios.isAxiosError(error)) {
      ...
      const errorMessage = error.response?.data?.message || error.message || "Failed to fetch self vote list";
